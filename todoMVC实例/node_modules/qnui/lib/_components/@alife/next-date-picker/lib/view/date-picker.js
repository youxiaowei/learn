...
var DatePicker = (_temp = _class = function (_Component) {
    ...
    DatePicker.prototype.componentWillReceiveProps = function componentWillReceiveProps(nextProps) {
        if ('value' in nextProps) {
            var normalizeValue = this.normalizeValue(nextProps.value);
    ...
    DatePicker.prototype.onDateChange = function onDateChange(nextState) {
        ...
        if (value) {
            ...
            // 非时间选择 及 选择今日 对时间值进行格式化
            if (from !== 'select-time' && !this.state.value && (0, _nextMoment2['default'])().format(formater[0]) !== normalizeValue.format(formater[0])) {
                // 判断用户是否传入了时间默认值，如果传入了，则使用该默认值初始化时间，如果没有，则格式化为 00:00:00
