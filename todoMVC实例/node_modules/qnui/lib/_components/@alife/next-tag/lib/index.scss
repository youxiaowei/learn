...
.#{$css-prefix} {
    ...
    &tag-selectable {
        ...
        &.#{$css-prefix}tag-small {
            @include selectable-tag-tick-size($tag-size-s-tick-icon-bg-size, $tag-size-s-tick-icon-size);
        }

        &.#{$css-prefix}tag-medium {
            @include selectable-tag-tick-size($tag-size-m-tick-icon-bg-size, $tag-size-m-tick-icon-size);
        }
    ...
    &tag-interactive {
        ...
        & .#{$css-prefix}tag-opt {
            ...
            .clicked {
                animation: zoomIn 0.3s $ease-in-out-circ;
            }
        }
    ...
    &tag-zoom-leave {
        animation: fadeOutLeft .2s $ease-in-out-circ;
        animation-fill-mode: both;
    }
}
